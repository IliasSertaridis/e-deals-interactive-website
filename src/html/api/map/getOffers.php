...
if(isset($_GET['store_id'])) {
    $results = DBQuery("SELECT item.item_id AS item_id, item.name AS name, offer.price AS price, offer.registration_date AS registered, offer.expiration_date AS expires, offer.number_of_likes AS likes, offer.number_of_dislikes AS dislikes, offer.in_stock AS in_stock FROM item, offer WHERE offer.store_id = '" .$_GET['store_id']. "' AND offer.item_id = item.item_id;");
